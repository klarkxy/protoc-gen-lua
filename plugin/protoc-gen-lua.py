...
DEBUG = True
logging.basicConfig(filename=os.path.join(PWD, "log.txt"),
...
def code_gen_message(message_descriptor : descriptor_pb2.DescriptorProto, env : TreeNode, containing_type : bool = None):
...
def code_gen_file(proto_file: descriptor_pb2.FileDescriptorProto, env: TreeNode, is_gen: bool):
    logging.info(f"开始解析{proto_file.name}")
    filename = os.path.splitext(proto_file.name)[0]
    env.enter_file(filename, proto_file.package)

    includes = []
    for f in proto_file.dependency:
        inc_file = os.path.splitext(f)[0]
    ...
    if is_gen:
        ...
        for i in includes:
            module_name = i.split("/")
            lua(f'local {module_name[-1]}_pb = require("{".".join(module_name)}_pb")\n')
        module_name = env.filename.split("/")
        lua(f"module('{'.'.join(module_name)}_pb')")
        
        lua('\n\n')
        for it in env.descriptor:
            lua(it)
        lua('\n')
        for it in env.context:
            lua(it)
        lua('\n')
        for it in env.message:
            lua(it)
        lua('\n')
        for it in env.register:
            lua(it)

        content = lua.getvalue()
        _files[env.filename + '_pb.lua'] = content
